...
var saleTxid =
  '0x5a7a930b16020723a05a9e467749a7ef5c1316e5a97eab2651c586a9deb4547d';
...
describe('BridgeSale contract', function () {
  ...
  it('Confirm transaction in block with proof', async function () {
    ...
    expect(lib.toHexString(trie.root)).to.equal(
      saleBlockData['transactionsRoot']
    );
  ...
  it('Do BridgeSale', async function () {
    ...
    BridgeSale = await BridgeSaleFactory.deploy(
      Bridge.address,
      '0xd000000000000000000000000000000000000b1e',
      787
    );

    // fund the bridgesale with 1 ETH
    await owner.sendTransaction({
      to: BridgeSale.address,
    ...
    await BridgeSale.redeemDeposit(
      lib.getBlockRlp(saleBlockData),
