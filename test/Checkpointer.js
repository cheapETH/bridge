...
describe('Checkpointer contract', function () {
  before(async function () {
    ...
    for (var i = 0; i < 110; i++) {
      await deployer.sendTransaction({
        to: '0xd000000000000000000000000000000000000b1e',
        value: 1,
      });
    }
  ...
  it('Submit checkpoint', async function () {
    const submit_block = await ethers.provider.getBlock(
      latest_block_number - 100
    );
    await Checkpointer.attest(submit_block.number, submit_block.hash);

    const returned_block_hash = await Checkpointer.getBlockByNumber(
      submit_block.number,
      deployer.address
    );
  ...
  it('Submit checkpoint and send to BridgeAuthority', async function () {
    ...
    const tx_block = await ethers.provider.getBlock(
      returned_block_hash.savedBlockNumber.toNumber()
    );
